  ...
  for (let l = 0; l < network.length; l++) {
    ...
    it(
      ...
      async () => {
        ...
        do {
          try {
            let output = await testNetSdk.getGatewaySubmittedBatch({
              hash: hashAddressBig,
            });
        ...
        try {
          output = await testNetSdk.getGatewaySubmittedBatch({
            hash: hashAddressBig,
          });
        ...
        try {
          singleTransaction = await testNetSdk.getTransaction({
        ...
        try {
          transactions = await testNetSdk.getTransactions();
