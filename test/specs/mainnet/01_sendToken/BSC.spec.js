...
describe("The SDK, when sending a native token with bsc network on the MainNet", () => {
  ...
  it("REGRESSION: Setup the SDK for bsc network and perform the send native token with invalid to address", async () => {
    // initialize the sdk
    try {
      bscMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
    ...
    // Compute the smart wallet address
    try {
      let smartWalletOutput = await bscMainNetSdk.computeContractAccount();
      let bscSmartWalletAddress = smartWalletOutput.address;
  ...
  it("REGRESSION: Setup the SDK for bsc network and perform the send native token with invalid value", async () => {
    // initialize the sdk
    try {
      bscMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Setup the SDK for bsc network and perform the send native token with exceeded value.", async () => {
    // initialize the sdk
    try {
      bscMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Setup the SDK for bsc network and perform the send native token on the same address", async () => {
    // initialize the sdk
    try {
      bscMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Setup the SDK for bsc network and perform the send native token without estimation of the batch", async () => {
    // initialize the sdk
    try {
      bscMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
