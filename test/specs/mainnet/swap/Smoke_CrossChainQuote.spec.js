...
describe("The SDK, when cross chain quote flow on the MainNet", () => {
  ...
  it.only("Setup the SDK for Mainnet network and perform the cross chain quote action", async () => {
    // initialize the sdk
    sdkMainNet = new Sdk(process.env.PRIVATE_KEY, {
      env: EnvNames.MainNets,
    ...
    let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Mainnet];
  ...
  it("Setup the SDK for xDai network and perform the cross chain quote action", async () => {
    // initialize the sdk
    sdkMainNet = new Sdk(process.env.PRIVATE_KEY, {
      env: EnvNames.MainNets,
