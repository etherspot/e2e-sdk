...
describe('Get the transaction history with matic network on the MainNet', function () {
  ...
  it('SMOKE: Perform the send native token on the matic network and get the transaction history', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        do {
          try {
            let output = await maticMainNetSdk.getGatewaySubmittedBatch({
              hash: hashAddressBig,
            });
        ...
        if (!(transactionState === 'Reverted')) {
          ...
          try {
            output = await maticMainNetSdk.getGatewaySubmittedBatch({
              hash: hashAddressBig,
            });
  ...
  it('REGRESSION: Perform the send native token on the matic network and get the transaction history from the random hash', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Fetching historical transactions
  ...
  it('REGRESSION: Perform the send native token on the matic network and get the transaction history with incorrect hash', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Fetching a single transaction
        try {
          let output = await maticMainNetSdk.getTransaction({
  ...
  it('REGRESSION: Perform the send native token on the matic network and get the transaction history when hash hex is not with 32 size', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Fetching a single transaction
