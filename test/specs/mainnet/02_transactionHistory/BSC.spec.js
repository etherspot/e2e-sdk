...
describe('Get the transaction history with bsc network on the MainNet', function () {
  ...
  it('SMOKE: Perform the send native token on the bsc network and get the transaction history', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        do {
          try {
            let output = await bscMainNetSdk.getGatewaySubmittedBatch({
              hash: hashAddressBig,
            });
        ...
        if (!(transactionState === 'Reverted')) {
          ...
          try {
            output = await bscMainNetSdk.getGatewaySubmittedBatch({
              hash: hashAddressBig,
            });
          ...
          try {
            singleTransaction = await bscMainNetSdk.getTransaction({
          ...
          try {
            transactions = await bscMainNetSdk.getTransactions();
  ...
  it('REGRESSION: Perform the send native token on the bsc network and get the transaction history from the random hash', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Fetching historical transactions
  ...
  it('REGRESSION: Perform the send native token on the bsc network and get the transaction history with incorrect hash', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Fetching a single transaction
        try {
          let output = await bscMainNetSdk.getTransaction({
  ...
  it('REGRESSION: Perform the send native token on the bsc network and get the transaction history when hash hex is not with 32 size', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Fetching a single transaction
        try {
          try {
            await bscMainNetSdk.getTransaction({
              hash: data.invalid_hash, // Incorrect Transaction Hash
            });
            addContext(
              test,
