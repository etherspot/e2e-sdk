...
let ArbitrumUSDT = "0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9"; // Arbitrum - USDT

let arbitrumMainNetSdk;
let arbitrumSmartWalletAddress;
let arbitrumSmartWalletOutput;

describe("The SDK, when swap the token with different features on the MainNet", () => {
  it("SMOKE: Perform the single chain swap action on the arbitrum network", async () => {
    ...
    try {
      offers = await arbitrumMainNetSdk.getExchangeOffers({
        fromTokenAddress: ArbitrumUSDC, // USDC Token
        toTokenAddress: ArbitrumUSDT, // USDT Token
  ...
  it("SMOKE: Perform the cross chain quote action on the arbitrum network", async () => {
    // initialize the sdk
    try {
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("SMOKE: Perform the advance routes lifi action on the arbitrum network", async () => {
    // initialize the sdk
    try {
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
    ...
    try {
      ...
      if (advanceRoutesLiFi.items.length > 0) {
        ...
        assert.fail("Not getting the items in the advanceRoutesLiFi response.");
      }
    } catch (e) {
      console.log(e);
      assert.fail(
  ...
  it("REGRESSION: Perform the single chain swap action without estimation of the batch on the arbitrum network", async () => {
    let transactionDetails;
    // Initialize the SDK and define network
    try {
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
  ...
  it("REGRESSION: Perform the single chain swap action from ERC20 token to ERC20 Token with exceed token balance on the arbitrum network", async () => {
    // Initialize the SDK and define network
    try {
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the single chain swap action from ERC20 token to the same ERC20 token on the arbitrum network", async () => {
    ...
    try {
      try {
        await arbitrumMainNetSdk.getExchangeOffers({
          fromTokenAddress: ArbitrumUSDC, // USDC Token
          toTokenAddress: ArbitrumUSDC, // Both are Same USDC Tokens
          fromAmount: ethers.utils.parseUnits("0.0001", 6),
        });
        assert.fail(
  ...
  it("REGRESSION: Perform the single chain swap action without toTokenAddress value while get the exchange offers on the arbitrum network", async () => {
    // Initialize the SDK and define network
    try {
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the single chain swap action without fromTokenAddress value while get the exchange offers on the arbitrum network", async () => {
    // Initialize the SDK and define network
    try {
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the single chain swap action without fromAmount value while get the exchange offers on the arbitrum network", async () => {
    // Initialize the SDK and define network
    try {
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the single chain swap action with invalid toTokenAddress value while get the exchange offers on the arbitrum network", async () => {
    // Initialize the SDK and define network
    try {
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the single chain swap action with invalid fromTokenAddress value while get the exchange offers on the arbitrum network", async () => {
    // Initialize the SDK and define network
    try {
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action without fromChainId value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action without toChainId value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action without fromTokenAddress value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action without toTokenAddress value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action without fromAmount value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action from native token to another chain's ERC20 token in the quote request payload on the arbitrum network", async () => {
    ...
    // Get the cross chain quotes
    try {
      let quotes = await arbitrumMainNetSdk.getCrossChainQuotes(
        quoteRequestPayload
      );
  ...
  it("REGRESSION: Perform the cross chain quote action with the same ERC20 tokens in the quote request payload on the arbitrum network", async () => {
    ...
    let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Arbitrum];
    let toChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Xdai];
    let fromTokenAddress = ArbitrumUSDC; // Arbitrum - USDC
    let toTokenAddress = ArbitrumUSDC; // Arbitrum - USDC
    let fromAmount = ethers.utils.parseUnits("0.5", 6);
  ...
  it("REGRESSION: Perform the cross chain quote action with exceeded token balance in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action with low token balance in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action without estimation of the batch on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action with invalid tokenAddress of the approval transaction request on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action with invalid approvalAddress of the approval transaction request on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action with invalid amount of the approval transaction request on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the cross chain quote action with invalid To Address of the approval transaction payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action without fromChainId value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action without toChainId value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action without fromTokenAddress value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action without toTokenAddress value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action without fromAmount value in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action from native token to another chain's ERC20 token in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action from ERC20 token to another chain's native token in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action with the same ERC20 tokens in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action with exceeded token balance in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action with low token balance in the quote request payload on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
  ...
  it("REGRESSION: Perform the advance route lifi action without estimation of the batch on the arbitrum network", async () => {
    try {
      // initialize the sdk
      arbitrumMainNetSdk = new Sdk(process.env.PRIVATE_KEY, {
        env: EnvNames.MainNets,
