...
let XdaiUSDT = "0x4ECaBa5870353805a9F068101A40E0f32ed605C6"; // Xdai - USDT
...
describe("The SDK, when swap the token with different features on the MainNet", () => {
  it("SMOKE: Perform the single chain swap action on the xdai network", async () => {
    ...
    try {
      offers = await xdaiMainNetSdk.getExchangeOffers({
        fromTokenAddress: XdaiUSDC, // USDC Token
        toTokenAddress: XdaiUSDT, // USDT Token
  ...
  it("REGRESSION: Perform the single chain swap action from ERC20 token to Native Token on the xdai network", async () => {
    ...
    try {
      xdaiSmartWalletOutput = await xdaiMainNetSdk.computeContractAccount();
      let xdaixdaiSmartWalletAddress = xdaiSmartWalletOutput.address;

      assert.strictEqual(
        xdaixdaiSmartWalletAddress,
        "0x666E17ad27fB620D7519477f3b33d809775d65Fe",
        "The smart wallet address is not calculated correctly."
      );
    } catch (e) {
  ...
  it("REGRESSION: Perform the single chain swap action from ERC20 token to the same ERC20 token on the xdai network", async () => {
    ...
    try {
      try {
        await xdaiMainNetSdk.getExchangeOffers({
          fromTokenAddress: XdaiUSDC, // USDC Token
          toTokenAddress: XdaiUSDC, // Both are Same USDC Tokens
          fromAmount: ethers.utils.parseUnits("0.0001", 6),
        });
        assert.fail(
