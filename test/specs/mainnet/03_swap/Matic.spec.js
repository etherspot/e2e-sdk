...
describe('The SDK, when swap the token with different features with the Matic network on the MainNet', () => {
  ...
  xit('SMOKE: Perform the advance routes lifi action on the Matic network', async () => {
    if (runTest) {
      // Prepare the quoteRequest Payload
      let quoteRequestPayload;
      try {
  ...
  xit('REGRESSION: Perform the advance route lifi action without fromChainId value in the quote request payload on the Matic network', async () => {
    if (runTest) {
      // Prepare the quoteRequest Payload
      let quoteRequestPayload;
      let toChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Xdai]; // without fromChainId
  ...
  xit('REGRESSION: Perform the advance route lifi action without toChainId value in the quote request payload on the Matic network', async () => {
    if (runTest) {
      // Prepare the quoteRequest Payload
      let quoteRequestPayload;
      let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic]; // without toChainId
  ...
  xit('REGRESSION: Perform the advance route lifi action without fromTokenAddress value in the quote request payload on the Matic network', async () => {
    if (runTest) {
      // Prepare the quoteRequest Payload
      let quoteRequestPayload;
      let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic];
  ...
  xit('REGRESSION: Perform the advance route lifi action without toTokenAddress value in the quote request payload on the Matic network', async () => {
    if (runTest) {
      // Prepare the quoteRequest Payload
      let quoteRequestPayload;
      let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic];
  ...
  xit('REGRESSION: Perform the advance route lifi action without fromAmount value in the quote request payload on the Matic network', async () => {
    if (runTest) {
      // Prepare the quoteRequest Payload
      let quoteRequestPayload;
      let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic];
  ...
  xit("REGRESSION: Perform the advance route lifi action from native token to another chain's ERC20 token in the quote request payload on the Matic network", async () => {
    if (runTest) {
      // Prepare the quoteRequest Payload
      let quoteRequestPayload;
      let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic];
  ...
  xit('REGRESSION: Perform the advance route lifi action with the same ERC20 tokens in the quote request payload on the Matic network', async () => {
    if (runTest) {
      // Prepare the quoteRequest Payload
      let quoteRequestPayload;
      let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic];
  ...
  xit('REGRESSION: Perform the advance route lifi action with low token balance in the quote request payload on the Matic network', async () => {
    if (runTest) {
      // Prepare the quoteRequest Payload
      let quoteRequestPayload;
      let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic];
