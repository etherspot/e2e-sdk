...
let maricSmartWalletOutput;
let maticNativeAddress = null;
let runTest;

describe('The SDK, when swap the token with different features with the Matic network on the MainNet', function () {
  beforeEach('Checking the sufficient wallet balance', async function () {
    ...
    try {
      maricSmartWalletOutput = await maticMainNetSdk.computeContractAccount();
      maticSmartWalletAddress = maricSmartWalletOutput.address;
  ...
  it('SMOKE: Perform the single chain swap action on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        try {
          offers = await maticMainNetSdk.getExchangeOffers({
            fromTokenAddress: data.maticUsdcAddress, // USDC Token
            toTokenAddress: data.maticUsdtAddress, // USDT Token
  ...
  it('SMOKE: Perform the cross chain quote action on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        ...
        let quotes;
        try {
          quotes = await maticMainNetSdk.getCrossChainQuotes(
            quoteRequestPayload
          );
  ...
  it('SMOKE: Perform the advance routes lifi action on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
        ...
        try {
          advanceRoutesLiFi = await maticMainNetSdk.getAdvanceRoutesLiFi(
            quoteRequestPayload
          );
          ...
          if (advanceRoutesLiFi.items.length > 0) {
            ...
            if (advanceRoutesLiFi.items.length > 0) {
              ...
              let transactions = await maticMainNetSdk.getStepTransaction({
  ...
  it('REGRESSION: Perform the single chain swap action from Native Token to ERC20 token on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        let transactionDetails;
  ...
  it('REGRESSION: Perform the single chain swap action from ERC20 token to ERC20 Token with exceed token balance on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Get exchange offers
  ...
  it('REGRESSION: Perform the single chain swap action from ERC20 token to the same ERC20 token on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Get exchange offers
        try {
          try {
            await maticMainNetSdk.getExchangeOffers({
              fromTokenAddress: data.maticUsdcAddress, // USDC Token
              toTokenAddress: data.maticUsdcAddress, // Both are Same USDC Tokens
              fromAmount: ethers.utils.parseUnits(
  ...
  it('REGRESSION: Perform the single chain swap action without toTokenAddress value while get the exchange offers on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Get exchange offers
        try {
          try {
            await maticMainNetSdk.getExchangeOffers({
              fromTokenAddress: data.maticUsdcAddress, // USDC Token and without toTokenAddress
              fromAmount: ethers.utils.parseUnits(
                data.singlechainswap_value,
                6
              ),
  ...
  it('REGRESSION: Perform the single chain swap action without fromTokenAddress value while get the exchange offers on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Get exchange offers
        try {
          try {
            await maticMainNetSdk.getExchangeOffers({
              toTokenAddress: data.maticUsdtAddress, // USDT Token and without fromTokenAddress
              fromAmount: ethers.utils.parseUnits(
                data.singlechainswap_value,
                6
              ),
  ...
  it('REGRESSION: Perform the single chain swap action without fromAmount value while get the exchange offers on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Get exchange offers
        try {
          try {
            await maticMainNetSdk.getExchangeOffers({
              fromTokenAddress: data.maticUsdcAddress, // USDC Token
              toTokenAddress: data.maticUsdtAddress, // USDT Token and without fromAmount
            });
            addContext(
              test,
  ...
  it('REGRESSION: Perform the single chain swap action with invalid toTokenAddress value while get the exchange offers on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Get exchange offers
        try {
          try {
            await maticMainNetSdk.getExchangeOffers({
              fromTokenAddress: data.maticUsdcAddress, // USDC Token
              toTokenAddress: data.invalid_maticUsdtAddress, // Invalid USDT Token
              fromAmount: ethers.utils.parseUnits(
                data.singlechainswap_value,
                6
              ),
  ...
  it('REGRESSION: Perform the single chain swap action with invalid fromTokenAddress value while get the exchange offers on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Get exchange offers
        try {
          try {
            await maticMainNetSdk.getExchangeOffers({
              fromTokenAddress: data.invalid_maticUsdcAddress, // Invalid USDC Token
              toTokenAddress: data.maticUsdtAddress, // USDT Token
              fromAmount: ethers.utils.parseUnits(
                data.singlechainswap_value,
                6
  ...
  it('REGRESSION: Perform the cross chain quote action without fromChainId value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the cross chain quote action without toChainId value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
        let quoteRequestPayload;
        let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic]; // without toChainId
        let fromTokenAddress = data.maticUsdcAddress; // USDC Token
        let toTokenAddress = data.xdaiUsdcAddress; // USDC Token
        let fromAmount = ethers.utils.parseUnits(data.crosschainswap_value, 6);
  ...
  it('REGRESSION: Perform the cross chain quote action without fromTokenAddress value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the cross chain quote action without toTokenAddress value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
        ...
        let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic];
        let toChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Xdai];
        let fromTokenAddress = data.maticUsdcAddress; // USDC Token and without toTokenAddress
        let fromAmount = ethers.utils.parseUnits(data.crosschainswap_value, 6);
  ...
  it('REGRESSION: Perform the cross chain quote action without fromAmount value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the cross chain quote action with the same ERC20 tokens in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
        ...
        let fromChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Matic];
        let toChainId = NETWORK_NAME_TO_CHAIN_ID[NetworkNames.Xdai];
        let fromTokenAddress = data.maticUsdcAddress; // Matic - USDC Token
        let toTokenAddress = data.maticUsdcAddress; // Matic - USDC Token
        let fromAmount = ethers.utils.parseUnits(data.crosschainswap_value, 6);
        ...
        // Get the cross chain quotes
        try {
          let quotes = await maticMainNetSdk.getCrossChainQuotes(
            quoteRequestPayload
          );
  ...
  it('REGRESSION: Perform the cross chain quote action with exceeded token balance in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the cross chain quote action with invalid tokenAddress of the approval transaction request on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the cross chain quote action with invalid amount of the approval transaction request on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the cross chain quote action with invalid To Address of the approval transaction payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the advance route lifi action without fromChainId value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the advance route lifi action without toChainId value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the advance route lifi action without fromTokenAddress value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the advance route lifi action without toTokenAddress value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the advance route lifi action without fromAmount value in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it("REGRESSION: Perform the advance route lifi action from native token to another chain's ERC20 token in the quote request payload on the Matic network", async function () {
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
        let quoteRequestPayload;
        ...
        // Get the advance routes lifi
        try {
          let advanceRoutesLiFi = await maticMainNetSdk.getAdvanceRoutesLiFi(
            quoteRequestPayload
          );
      ...
      console.warn(
        "DUE TO INSUFFICIENT WALLET BALANCE, SKIPPING TEST CASE OF THE ADVANCE ROUTE LIFI ACTION FROM NATIVE TOKEN TO ANOTHER CHAIN'S ERC20 TOKEN IN THE QUOTE REQUEST PAYLOAD ON THE Matic NETWORK"
      );
    }
  });

  it('REGRESSION: Perform the advance route lifi action with the same ERC20 tokens in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        // Prepare the quoteRequest Payload
  ...
  it('REGRESSION: Perform the advance route lifi action with low token balance in the quote request payload on the Matic network', async function () {
    var test = this;
    if (runTest) {
      await customRetryAsync(async function () {
        try {
